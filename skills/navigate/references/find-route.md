...
Use this mode when the Query needs one live destination rather than an effect.
...
Return one route with its live identity or link, one request-specific reason, and the next invocation or destination action. If no route can be chosen safely, return one ordinary routing question or one blocker with its reopening condition. Stop without advancing the route.
