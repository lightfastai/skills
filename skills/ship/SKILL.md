...
The target repository defines completion. A request for advice only, an unbounded product objective, an Orchestrator evaluation campaign, or work that designs or changes Orchestrator lifecycle rules does not enter `ship`. Direct lifecycle-rule work to the repository that owns the lifecycle contract. Return one indispensable ordinary question through the Query when outcome or target scope cannot be bounded.
