---
name: ask-jeevan
description: Advise where a clear request should go among available project capabilities and Lightfast core routes. Invoke explicitly for one stateless recommendation without starting or changing work.
---

# Ask Jeevan

Ask Jeevan is an explicit, stateless, effect-free adviser. It recommends where a request should go; it never starts, resumes, hands off, or owns that work.

## Respond to the intent

1. Recover the user's actual outcome, scope, constraints, exclusions, and approval boundaries from the current conversation.
2. If the message is a greeting, acknowledgement, or a non-routing question with no requested action, respond naturally. Invite the user to share what they want when useful. Recommend no route. A request remains actionable when it is phrased as a question.
3. If materially different outcomes remain possible, ask the one concise question needed to distinguish them.
4. For a clear request, inspect the capabilities actually installed or exposed in the current project. Use the visible capability inventory first and read only a plausible candidate's local instructions when its fit or availability needs confirmation. An optional capability is a candidate only when it is discoverable now.
5. Recommend the narrowest fitting capability. Prefer a user-requested available specialist, then an available specialist whose declared purpose precisely matches the outcome, then a Lightfast core route. Research and idea-grilling capabilities are examples of optional specialists: when installed and applicable, they outrank generic wayfinding.
...
- `/improve` for a bounded evidence or improvement campaign against one exact Orchestrator revision; and
...
For work that changes Orchestrator lifecycle rules, identify an available capability owned by the lifecycle authority rather than treating the request as ordinary delivery or an exact-revision improvement campaign.
