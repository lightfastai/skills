...
A **flow** is the path from an originating **Query** conversation to one bounded outcome and back to evidence-based reconciliation. The public v1 flow has three routes. This skill recommends where to enter; it is stateless, performs no effect, and never enters the route itself.
...
2. **Enter the closest outcome route directly:**
...
- **The request changes how Orchestrators are created, evaluated, checked for conformance, or upgraded** → identify the lifecycle authority. Lifecycle authority takes precedence over an exact-revision improvement campaign.
...
- **An accepted code outcome is ready for repository-native delivery** → `/ship`. Work whose outcome is an Orchestrator lifecycle-rule change belongs to the lifecycle authority.
